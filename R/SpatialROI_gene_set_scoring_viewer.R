...
#'
#' @rawNamespace import(shiny, except = c(runExample))
#' @import leaflet
#' @rawNamespace import(Seurat, except = c(JS))
#' @import ggplot2
...
run_gene_set_viewer <- function(seurat_obj, sample_name = "Sample", show_image = TRUE) {
  ...
  ui <- fluidPage(
    ...
    tags$div(class = "top-header",
             tags$h1("🧬 SpatialROI - Gene Set Analyzer"),
                      ...
                      tags$div(class = "control-section",
                               ...
                               tags$p(style = "font-size: 11px; color: #7f8c8d; margin: -8px 0 10px 0;",
                                      HTML("<b>Mean</b>: Fastest, simple average<br>
                                            ...
                                            <b>Rank-based</b>: per-spot mean-rank enrichment, scaled to [-1, 1] (may take 10-30s)")),
  ...
  server <- function(input, output, session) {
    ...
    observeEvent(input$calculate_btn, {
      ...
      withProgress(message = 'Calculating scores...', value = 0, {
        ...
        } else if (input$score_method == "rank") {
          # Fast mean-rank enrichment score; this is not the GSVA algorithm.
